...
const staticChannelPerformance = [
...
const staticContentPerformance = [
...
export default function AnalyticsPage() {
  ...
  const engagementData = data?.engagementData || staticEngagementData;
  const growthData = data?.growthData || staticGrowthData;
  const channelPerformance = data?.channelPerformance || staticChannelPerformance;
  const contentPerformance = data?.contentPerformance || staticContentPerformance;
  const healthScore = [{ name: "score", value: data?.healthScore || 78, fill: "hsl(158 64% 52%)" }];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Reach" value={formatNumber(overview.totalReach)} delta={overview.reachDelta} deltaLabel="vs prev period" icon={<Eye className="h-5 w-5" />} chart={<Sparkline data={[30,40,35,50,45,60,55,68]} color="hsl(243 75% 59%)" />} />
        <StatCard label="Avg. Engagement" value={`${overview.avgEngagement}%`} delta={overview.engagementDelta} deltaLabel="vs prev period" icon={<Heart className="h-5 w-5" />} chart={<Sparkline data={[3,4,3.5,5,4.5,6,5.5,6.2]} color="hsl(158 64% 52%)" />} />
        <StatCard label="Conversions" value={formatNumber(overview.conversions)} delta={overview.conversionsDelta} deltaLabel="vs prev period" icon={<Zap className="h-5 w-5" />} chart={<Sparkline data={[10,22,18,30,25,35,40,48]} color="hsl(38 92% 50%)" />} />
        <StatCard label="New Followers" value={formatNumber(overview.followers)} delta={overview.followersDelta} deltaLabel="vs prev period" icon={<Users className="h-5 w-5" />} chart={<Sparkline data={[40,38,45,42,48,44,50,46]} color="hsl(262 83% 58%)" />} />
      </div>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={channelPerformance} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3}>
                  {channelPerformance.map((e: any) => <Cell key={e.name} fill={e.color} />)}
                </Pie>
                <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid hsl(var(--border))", background: "hsl(var(--card))", fontSize: 12 }} />
              </PieChart>
            </ResponsiveContainer>
            <div className="mt-3 space-y-2">
              {channelPerformance.map((c: any) => (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {campaignsList.filter((c: any) => c.status === "active" || c.status === "completed").map((c: any) => {
