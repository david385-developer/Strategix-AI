...
import { teamMembers as staticMembers, tasks as staticTasks, activities as staticActivities } from "@/lib/data";
...
export default function TeamPage() {
  ...
  const { teamMembers: dbMembers, inviteMember } = useWorkspace();
  const { tasks: dbTasks } = useTeam();
  const { data: dashboardData } = useDashboard();

  const members = dbMembers && dbMembers.length > 0 ? dbMembers : staticMembers;
  const tasks = dbTasks && dbTasks.length > 0 ? dbTasks : staticTasks;
  const activities = dashboardData?.activities && dashboardData.activities.length > 0 ? dashboardData.activities : staticActivities;

  const handleInvite = async () => {
    const email = prompt("Enter team member email:");
    if (!email) return;
    const name = prompt("Enter team member name:") || email.split("@")[0];
    const role = prompt("Enter role (admin/editor/viewer):") || "viewer";

    try {
      await inviteMember({ name, email, role });
      toast.success("Invitation sent!", `Successfully invited ${name} as a ${role}.`);
    } catch (err: any) {
      toast.error("Invite failed", err.response?.data?.message || "Failed to invite member.");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={handleInvite}><UserPlus className="h-4 w-4" /> Invite member</Button>}
      />

      {/* Stats */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Members", value: members.length, icon: UserPlus, color: "text-primary" },
          { label: "Open tasks", value: tasks.filter((t: any) => t.status !== "done").length, icon: ListTodo, color: "text-chart-2" },
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {members.map((m: any) => {
                    const role = roleConfig[m.role as MemberRole] || { className: "bg-muted text-muted-foreground", label: m.role };
                    return (
                      <tr key={m.id || m._id} className="transition-colors hover:bg-muted/30">
              ...
              <div key={col.title} className="space-y-3">
                ...
                <div className="space-y-2.5">
                  {col.tasks.length === 0 ? (
                    ...
                    <Card key={t.id || t._id} className="cursor-pointer transition-all hover:shadow-soft">
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          ...
                          <Avatar className="h-6 w-6">
                            <AvatarFallback className="bg-primary/10 text-[9px] text-primary">
                              {((t.assignee?.name || t.assignee || "Sarah Chen") + "").split(" ").map((p) => p[0]).join("").slice(0, 2)}
                            </AvatarFallback>
                          </Avatar>
                        </div>
                      </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-1">
              {activities.map((a: any) => (
                <div key={a.id || a._id} className="flex items-center gap-3 rounded-lg p-3 transition-colors hover:bg-muted/40">
                  <Avatar className="h-9 w-9">
