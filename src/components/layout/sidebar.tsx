        ...
        <AnimatePresence>
          {workspaceOpen && !collapsed && (
            ...
            >
              ...
              {["Strategix HQ", "Client: Acme Co", "Personal Brand"].map((w, i) => (
                <button
                  key={w}
                  className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-sm transition-colors hover:bg-muted"
                >
                  <div className={cn("h-6 w-6 rounded-md", i === 0 ? "bg-primary" : "bg-muted")} />
                  <span className={cn(i === 0 && "font-medium")}>{w}</span>
                </button>
              ))}
              <div className="my-1 h-px bg-border" />
              <button className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-sm text-primary transition-colors hover:bg-primary/5">
