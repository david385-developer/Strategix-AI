...
import { campaigns, contentItems, activities, tasks } from "@/lib/data";
...
export default function DashboardPage() {
  ...
  const upcomingPosts = contentItems.filter((c) => c.status === "scheduled" || c.status === "approval").slice(0, 4);
  const pendingApprovals = contentItems.filter((c) => c.status === "approval");
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="border-primary/20 bg-gradient-to-b from-primary/[0.03] to-transparent">
          ...
          <CardContent className="space-y-3">
            {[
              { icon: TrendingUp, text: "Post your Instagram Reels on Thursday at 6 PM for 28% more reach." },
              { icon: PenSquare, text: "Generate 3 LinkedIn posts about your webinar to boost registrations." },
              { icon: Activity, text: "Repurpose your top Twitter thread into a blog post this week." },
            ].map((r, i) => (
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-2.5">
            {upcomingPosts.map((item) => {
              const cfg = contentStatusConfig[item.status];
              return (
                <div
                  key={item.id}
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-1">
            {activities.slice(0, 6).map((a) => (
              <div key={a.id} className="flex items-center gap-3 rounded-lg p-2 transition-colors hover:bg-muted/40">
