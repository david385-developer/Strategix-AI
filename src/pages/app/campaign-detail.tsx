...
import { campaigns, contentItems, teamMembers } from "@/lib/data";
...
export default function CampaignDetailPage() {
  ...
  const cfg = campaignStatusConfig[campaign.status];
  const campaignContent = contentItems.filter((c) => c.campaign === campaign.name);
          ...
          {campaignContent.length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {campaignContent.map((item) => {
                ...
                const Icon = platformIcons[item.type] ?? ImageIcon;
                return (
        ...
        <TabsContent value="ai-strategy" className="space-y-4">
          <Card className="border-primary/20">
            <CardHeader className="flex-row items-center justify-between space-y-0">
              ...
              <Button variant="outline" size="sm"><Sparkles className="h-3.5 w-3.5" /> Regenerate</Button>
            </CardHeader>
            <CardContent className="space-y-5">
              {[
                { title: "Target Audience", desc: "Marketing managers and founders at B2B SaaS companies (50-500 employees) who want to scale content without growing headcount." },
                { title: "Marketing Funnel", desc: "Awareness → Teaser posts, Consideration → Educational content, Decision → Social proof + limited-time offer." },
                { title: "Content Pillars", desc: "1) Product storytelling 2) Customer success 3) Behind-the-scenes 4) Educational tips 5) Industry trends." },
                { title: "Posting Schedule", desc: "Instagram: 3x/week (Tue, Thu, Sun). Email: Weekly. Facebook: 2x/week. Best time: 10 AM & 6 PM." },
                { title: "Budget Suggestions", desc: "45% paid social, 30% content production, 15% influencer, 10% tools & automation." },
                { title: "KPIs", desc: "5,000 signups, 500K reach, 8% engagement rate, $2.40 cost per acquisition." },
              ].map((s) => (
